...
namespace CodeKicker.BBCode.Core.Tests
{
    public partial class BBCodeTest
    {
                ...
                var tree2 = BBCode.ReplaceTextSpans(tree1, txt =>
                    {
                        ...
                        return
                            Enumerable.Range(0, count)
                                .Select(x =>
                                    {
                                        var maxIndex = x == count - 1 ? txt.Length : indexes[x + 1];
                                        var text = RandomValue.String();
                                        chosenTexts.Add(text);
                                        return new TextSpanReplaceInfo(indexes[x], RandomValue.Int(indexes[x] - maxIndex + 1, 0), new TextNode(text));
                                    })
                                .ToArray();
                    }, null);
        ...
        [Theory]
        [InlineData("[b:8475h6jds]this is some bold text[/b:8475h6jds]", "<b>this is some bold text</b>")]
        [InlineData("[url:h75ks63nh5]https://google.com[/url:h75ks63nh5]", "<a href=\"https://google.com\" target=\"_blank\">https://google.com</a>")]
        [InlineData("[url=https://google.com:7845jh5674]this is a custom link[/url:7845jh5674]", "<a href=\"https://google.com\" target=\"_blank\">this is a custom link</a>")]
        [InlineData("[quote=\"someone\":474h4gfs]some text[/quote:474h4gfs]", "<blockquote class=\"PostQuote\"><b>someone</b> wrote:<br/>some text</blockquote>")]
        public void BbcodeUid_IsHandled(string text, string expectedHtml)
        {
            Assert.Equal(expectedHtml, HttpUtility.HtmlDecode(BBCodeTestUtil.GetCustomParser().ToHtml(text)));
        }

        [Theory]
        [InlineData("[b:8475h6jds][i:8475h6jds]this is some bold italic text[/i:8475h6jds][/b:8475h6jds]", "<b><i>this is some bold italic text</i></b>")]
        [InlineData("[b:7sh4g6b3j][url:7sh4g6b3j]https://google.com[/url:7sh4g6b3j][/b:7sh4g6b3j]", "<b><a href=\"https://google.com\" target=\"_blank\">https://google.com</a></b>")]
        [InlineData("[b:87th8gfr][url=https://google.com:87th8gfr]some text[/url:87th8gfr][/b:87th8gfr]", "<b><a href=\"https://google.com\" target=\"_blank\">some text</a></b>")]
        [InlineData("[b:3q85xb4n][size=200:3q85xb4n][color=red:3q85xb4n]aaa[/color:3q85xb4n][/size:3q85xb4n][/b:3q85xb4n]", "<b><span style=\"font-size:2em\"><span style=\"color:red\">aaa</span></span></b>")]
        [InlineData("[quote=\"someone\":474h4gfs][quote=\"someone else\":474h4gfs]some nested text[/quote:474h4gfs][/quote:474h4gfs]", "<blockquote class=\"PostQuote\"><b>someone</b> wrote:<br/><blockquote class=\"PostQuote\"><b>someone else</b> wrote:<br/>some nested text</blockquote></blockquote>")]
        public void BbcodeUid_WhenNested_IsHandled(string text, string expectedHtml)
        {
            Assert.Equal(expectedHtml, HttpUtility.HtmlDecode(BBCodeTestUtil.GetCustomParser().ToHtml(text)));
        }
